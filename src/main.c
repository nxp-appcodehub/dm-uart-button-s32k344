...
Siul2_Dio_Ip_GpioType * const LED_PORT[NUM_LEDS] = { LED_BLUE_PORT, LED_GREEN_PORT, LED_RED_PORT };
uint8 LED_PINS[NUM_LEDS] = { LED_BLUE_PIN, LED_GREEN_PIN, LED_RED_PIN };
...
void SendMessage(char Message[]) {
    ...
    for (uint32_t index = 0U; index < msgLen; index++) {
        TxBuff = (uint8_t) Message[index];
        /*
         * Wait for driver to be ready
         */
        while (Lpuart_Uart_Ip_GetTransmitStatus(INST_LP, &bytesRemaining)
                == LPUART_UART_IP_STATUS_BUSY)
            ;
        /*
         * Send data to console
         */
        Lpuart_Uart_Ip_SyncSend(INST_LP, &TxBuff, 1, 100000);
        /*
...
void BlinkBlueLed(void) {
    ...
    uint32 elapsed = 0U;
    uint32 timeout = OsIf_MicrosToTicks(50 * 1000U, OSIF_COUNTER_SYSTEM);
...
void ReceiveCommand(void) {
    ...
    status = Lpuart_Uart_Ip_GetReceiveStatus(INST_LP, &bytesRemaining);
    ...
    if (status == LPUART_UART_IP_STATUS_SUCCESS ||
        ...
        status = Lpuart_Uart_Ip_SyncReceive(INST_LP, RxBuffer, 1, 100000); // 100ms
        ...
        if (status == LPUART_UART_IP_STATUS_SUCCESS) {
            ...
            if (receivedChar == '\b' || receivedChar == 127) {
                ...
                Lpuart_Uart_Ip_SyncSend(INST_LP, &receivedChar, 1, 100000);
            }
        }
    }
}
...
void ProcessCommand(void) {
    ...
    /* * FIX: These must stay INSIDE the execution block,
     * otherwise they clear the buffer on loops where no data was received.
     */
    CommandIndex = 0;
    CommandReady = false;
...
void DelayMs(void) {
	uint32 cur = OsIf_GetCounter(OSIF_COUNTER_SYSTEM);
	uint32 elapsed = 0U;
	uint32 timeout = OsIf_MicrosToTicks(BTN_DELAY_TIME * 1000U, OSIF_COUNTER_SYSTEM);
	while (elapsed < timeout) {
		elapsed += OsIf_GetElapsed(&cur, OSIF_COUNTER_SYSTEM);
	}
}
...
int main(void) {
    ...
    Lpuart_Uart_Ip_Init(INST_LP, &Lpuart_Uart_Ip_xHwConfigPB_6);
        ...
        if(Siul2_Dio_Ip_ReadPin(SW2_PORT, SW2_PIN)){
            if(!sw2_pressed){
                ...
                if(GreenLedState){
                    GreenLedState = false;
                    Siul2_Dio_Ip_WritePin(LED_GREEN_PORT, LED_GREEN_PIN, 0);
                    SendMessage("\r\nGreen LED turned OFF (Button)\r\n> ");
                } else {
                    GreenLedState = true;
                    Siul2_Dio_Ip_WritePin(LED_GREEN_PORT, LED_GREEN_PIN, 1);
                    SendMessage("\r\nGreen LED turned ON (Button)\r\n> ");
                }
                DelayMs();
            }
        } else {
